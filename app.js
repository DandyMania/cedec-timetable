...
function sessionCard(s, terms, liveState, showDate) {
  ...
  const speakers = (s.speakers ?? [])
    .map(
      (x) =>
        // One link per speaker, at the LEFT of the line so the ↗ chips line up
        // in a column and stay clear of the card-tap area. It searches the
        // person (name + company); the company's own site stays a tap away in
        // the detail sheet. Names/companies are inert text.
        `<span class="sp"><a class="sp__go" href="${speakerNameSearchUrl(x)}" target="_blank" rel="noopener"
             title="${esc(x.name)}${x.company ? ` ${esc(x.company)}` : ''} を検索"
             aria-label="${esc(x.name)} を検索">↗</a><span class="sp__name">${highlight(x.name, terms)}</span>${
      ...
      const cells = dated
        ...
        .map((s) => {
          ...
          return `<div class="gcell cat-edge-${esc(s.category || 'none')} ${
            ...
            ${(() => {
              // The grid used to drop the name whenever a company existed. Show
              // both — company first, then the speaker name below it (DM's order).
              ...
              return `${cos ? `<div class="gcell__co">${esc(cos)}</div>` : ''}${
                names ? `<div class="gcell__sp">${esc(names)}</div>` : ''
              }`;
            })()}
